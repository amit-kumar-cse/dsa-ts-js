const alternatingSubArraysEasy = (A: number[], B: number) => {
    const prefixArr = new Array(A.length);
        prefixArr[0] = 1;
        for(let i=1; i<A.length; i++) {
            if(BigInt(A[i]) + BigInt(A[i-1]) === 1n) {
...
export { alternatingSubArraysEasy };
